 ...
 * cron 每天1次
 * 变量名:zsxxCookie
 * 变量值:http://zsxx.tuesjf.cn/ 网址后的token的值（去掉token=）
 * 完整格式 643545-f225-7542-7jk4-3434434324
 * 多账号 & 分割
 */


const $ = new Env("掌上修仙");
const ckName = "zsxxCookie";
...
class UserInfo {
    constructor(index, str) {
        ...
        this.getHeaders = {
            ...
            "User-Agent": "Mozilla/5.0 (Linux; Android 9; vivo X21A Build/PKQ1.180819.001; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/68.0.3440.91 Mobile Safari/537.36 uni-app Html5Plus/1.0 (Immersed/28.0)",
...
async function main(user) {
    ...
    if (userinfo_res) {
        await signNunShow(user);

        await upGvip(user);

        for (let i = 0; i < 5; i++) {
            let videoOrder = await getVideoOrder(user);
...
async function getUserInfo(user) {
    const user_info = await axios({
        method: 'get',
        url: `http://zsxx.tuesjf.cn/apis/v1/user_info?token=${user.token}`,
        headers: user.getHeaders,
    });

    if (user_info) {
        if (user_info.data.code == 1) {
...
async function signNunShow(user) {
    const res = await axios({
        method: 'get',
        url: `http://zsxx.tuesjf.cn/apis/v1/signNunShow?token=${user.token}`,
        headers: user.getHeaders,
    });
    ...
    if (res) {
        ...
        console.log(`账号${user.index}signNunShow错误:${res.data}`);
        return false;
    }
}

async function upGvip(user) {
    const res = await axios({
        method: 'post',
        url: `http://zsxx.tuesjf.cn/apis/v1/upGvip`,
        headers: user.getHeaders,
        data: {
            token: user.token,
        },
    ...
    if (res) {
        ...
        console.log(`账号${user.index}upGvip错误:${res.data}`);
        return false;
    }
}
